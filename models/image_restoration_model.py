...
class ImageCleanModel(BaseModel):
    ...
    def optimize_parameters(self, current_iter):
        ...
        # pixel loss
        l_pix = 0.60 * (1 - self.cri_pix1(xt_next, self.gt)) + self.cri_pix2(xt_next, self.gt)  # ssim+l1
        #  l_pix = 0.88 * (1 - self.cri_pix1(xt_next, self.gt)) + self.cri_pix2(xt_next, self.gt)  # ssim+l1
        # l_pix = 0.72 * (1 - self.cri_pix1(xt_next, self.gt)) + self.cri_pix2(xt_next, self.gt)  # ssim+l1
        # l_pix = 0.76 * (1 - self.cri_pix1(xt_next, self.gt)) + self.cri_pix2(xt_next, self.gt)  # ssim+l1
        # l_pix = 0.80 * (1 - self.cri_pix1(xt_next, self.gt)) + self.cri_pix2(xt_next, self.gt)  # ssim+l1
        l_pix = 0.92 * (1 - self.cri_pix1(xt_next, self.gt)) + self.cri_pix2(xt_next, self.gt)  # ssim+l1
        loss_dict['l_pix'] = l_pix
        l_pix.backward()
        if self.opt['train']['use_grad_clip']:
            torch.nn.utils.clip_grad_norm_(self.net_g.parameters(), 0.01)
